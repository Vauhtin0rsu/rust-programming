...
fn battle_normal(player: &mut Player, monsters: &mut Vec<Monster>) {
    ...
    let mut boss_multiplier: f32 = 1.0;
    boss_multiplier += 0.1 * player.bosses_won as f32; // make enemies stronger depending on how many bosses the player has beaten
    ...
    for monster in monsters {   // loop through the monster fights
        monster.hp = monster.hp + 5 * player.bosses_won; // give monsters more hope depending on the difficulty level
